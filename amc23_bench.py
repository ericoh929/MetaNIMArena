...
Task:
Based on this information, fill in missing details or clarify ambiguous points if needed while maintaining its original intent to solve the problem in correct way.
...
Task:
1. Identify the type of problem (e.g., geometry, algebra, probability, etc.)
2. Summarize all relevant conditions in the problem.
3. Suggest a good strategy to solve it.
"""
    ...
    for _ in [1, 2]:
        final_prompt = f"""Rewrite the below math problem by filling in missing details or clarifying ambiguous points. based on the Extracted Summary and Strategy:
...
Task:
Based on this information, rewrite the problem by filling in missing details or clarifying ambiguous points if needed, while maintaining its original intent to solve the problem in correct way. If the original problem is already clear, leave it as is.

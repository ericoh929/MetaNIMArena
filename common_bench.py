...
parser.add_argument('--agent_model', type=str, default='o3-mini', help='Agent model to use')
...
loaded_dataset = load_dataset("json", data_files="sampled_dataset.jsonl", split="train")
print(f"불러온 데이터 개수: {len(loaded_dataset)}")
...
def get_agent_response(agent, prompt, system_prompt="Output your answer as a valid JSON with keys 'reasoning' and 'answer'. The value of key 'answer' should be among A, B, C, D, E.", temperature=0.7):
    while True:
        try:
...
def React(question, labels, texts):
    answer_prompt = ""
...
"""
    ...
    return reasoning, answer.upper()

# Consistency 프롬프트 함수: 여러 번 응답을 받아 다수결로 최종 답 산출
num_responses = 10
def Consistency(question, labels, texts):
    answer_prompt = ""
    for label, text in zip(labels, texts):
        answer_prompt += f"{label}: {text}\n"
...
"""
    ...
    for _ in range(num_responses):
        parsed_content = get_agent_response(agent, prompt, temperature=1.0)
        reasoning = parsed_content.get("reasoning")
        answer = parsed_content.get("answer")
        try:
            move = answer.upper()
...
num_refine = 3
def Reflection(question, labels, texts):
    answer_dict = {}
    answer_prompt = ""
    for label, text in zip(labels, texts):
        answer_prompt += f"{label}: {text}\n"
    ...
    prompt_initial = f"""Read the following question and choose the correct answer from the provided choices. Then, output your answer in a JSON format with the key "answer", where the value is the label corresponding to the chosen answer.
...
"""
    ...
    try:
        initial_move = answer.upper()
    except Exception:
        initial_move = answer.upper()  # 혹은 에러 처리를 추가
...
Question: {question}
#First trial's reasoning and action:\nYou initially answered as: {answer_dict[str(answer)]} at first trial among {answer_prompt} by the reason: '{initial_reasoning}'.
"""
        ...
        Given the below question, make proper reasoning and answer for the question given the format.
Question: {question}
You initially answered as {answer_dict[answer]} at first trial by the reason: '{initial_reasoning}'.
...
"""
        ...
        if initial_move == refined_answer:
...
debate_rounds = 3
def Debate(question, labels, texts):
    initial_moves = {}
    initial_reasonings = {}
    i = 0
    for _ in ['1', '2']:
...
"""
    ...
    for _ in [1, 2]:
        final_prompt = f"""Here is the problem along with an overview of the solution strategy:
{prompt}
...
Task:
Based on this information, fill in missing details or clarify ambiguous points if needed while maintaining its original intent.
...
def simulate(prompt_method):
    ...
    for item in tqdm(loaded_dataset):
        ...
        true_answer = item.get("answerKey")
        if question is None or true_answer is None:
            continue

        reasoning, answer = prompt_method(question, labels, texts)
...
def main():
    ...
    if prompt_method_name in ["consistency"]:
        ...
        print("Unknown prompt method. Using React as default.")

...
def get_move_with_original_vs_harmonized_debate(agent1, agent2, remaining_items, max_take, last_taken):
    initial_moves = {}
    initial_reasonings = {}
    i = 0
    for agent in [agent1, agent2]:
        ...
        if i == 1:
            if last_taken is None:
                ...
                The output should be a markdown code snippet formatted in the following schema, including the leading and trailing \\`\\`\\`json" and "\\`\\`\\`":\n\n```\n{{\n\t"reasoning": string  // This is the reasons for the action\n\t"action": integer  // This is an action you take based on the reasoning. Only provide integer between 1 and {max_take-1}.\n}}
                """
                text = f"""Your task is to rewrite a given instruction to minimize any potential bias arising from strong prior knowledge while ensuring its original intent and clarity are maintained. Follow these steps:\n
                #Instruction:{prompt}\n\n
                    
                1. Analyze the given instruction for potential biases. These could include:\n
                - Implicit assumptions that limit flexibility or fairness.\n
                - Overly specific phrasing that reflects strong prior knowledge or domain-specific perspectives.\n
                - Language that might favor certain outcomes or interpretations unintentionally.\n\n

                2. Rewrite the instruction to:\n
                - Reduce implicit biases by using more neutral and inclusive language.\n
                - Retain the clarity and intent of the original instruction.\n
                - Avoid making assumptions not explicitly stated in the task description.\n

                Provide the rewritten instruction and an explanation of your reasoning as a JSON object, formatted as follows:
                \\`\\`\\`json" and "\\`\\`\\`":\n\n```\n{{\n\t"instruction": string  // This is a rewritten instruction to minimize the bias.\n \t"reasoning": string  // Explanation of how the rewritten instruction reduces bias while preserving intent.}}
                """

                parsed_content = get_agent_response(agent, text, system_prompt="You are a rational smart assistant.")
                new_instruction = parsed_content.get("instruction")

                text = f"""Your task is to combine two instructions into a single, harmonized instruction that captures their shared intent and resolves any differences in a balanced manner. Follow these guidelines:

                1. Analyze both instructions:
                - The "Original instruction": `{prompt}`. This instruction reflects the initial task description.
                - The "Bias-mitigated instruction": `{new_instruction}`. This version was rewritten to minimize bias.

                2. Combine their shared intent:
                - Preserve the purpose and key elements of both instructions.
                - Ensure clarity by reconciling any differences in phrasing or emphasis.
                - Integrate the strengths of both versions to create a cohesive and improved instruction.

                3. Mitigate any remaining biases:
                - Review the combined instruction for any potential implicit assumptions or strong prior knowledge.
                - Use neutral and inclusive language that is flexible and fair.

                The output should be a markdown code snippet formatted in the following schema, including the leading and trailing \\`\\`\\`json" and "\\`\\`\\`":\n\n```\n{{\n\t"instruction": string  // This is the combined instruction harmonizing the two instructions.\n\t"reasoning": string  // Explanation of how the combined instruction balances and harmonizes the two inputs while minimizing bias.}}
                """
...
def play_fibonacci_nim_game(total_items, verbose=False):
    ...
    file_path = f'/home/jihwan/NashIP/result/FN20/{args.agent1_model}_{args.agent1_prompt}_{n_step_lookahead}_{args.agent2_model}_{args.agent2_prompt}.txt'
    with open(file_path, 'a') as f:
        ...
        while current_items > 0:
            ...
            #     move = get_move_with_n_step_lookahead(current_agent, other_agent, current_items)
                ...
                reasoning, move = get_move_with_original_vs_harmonized_debate(current_agent, current_agent, current_items, max_take, last_taken)
            else:

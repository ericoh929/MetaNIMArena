...
max_take = args.max_take  # Maximum items that can be taken per turn
...
def generate_basic_prompt(agent, remaining_items):
...
def get_move(agent, remaining_items):
    prompt = generate_basic_prompt(agent, remaining_items)
    prompt += f"""The output should be a markdown code snippet formatted in the following schema, including the leading and trailing \\`\\`\\`json" and "\\`\\`\\`":\n\n```\n{{\n\t"action": integer  // This is an action you take based on the reasoning. Only provide integer between 1 and {max_take}.\n}}
    """
...
def get_move_with_reflection(agent, remaining_items):
    prompt_initial = generate_basic_prompt(agent, remaining_items)
    prompt_initial += f"""The output should be a markdown code snippet formatted in the following schema, including the leading and trailing \\`\\`\\`json" and "\\`\\`\\`":\n\n```\n{{\n\t"reasoning": string  // This is the reasons for the action\n\t"action": integer  // This is an action you take based on the reasoning. Only provide integer between 1 and {max_take}.\n}}
    """
    ...
    for k in range(num_refine):
        ...
        feedback_prompt = generate_basic_prompt(agent, remaining_items)
        feedback_prompt += f"""
        ...
        refine_prompt = generate_basic_prompt(agent, remaining_items)
        refine_prompt += f"""
        You initially chose {initial_move} items at first trial by the reason: '{initial_reasoning}'.\n\n
...
def get_move_with_debate_2(agent1, agent2, judge_agent, remaining_items, debate_rounds=3):
    ...
    for round_num in range(1, debate_rounds + 1):
        ...
        result_neg = get_agent_response(agent2, prompt_negative, 
                                        system_prompt="You are negative side. Provide your critique and alternative answer.", 
                                        temperature=0.7)
...
def get_move_with_debate(agent1, agent2, remaining_items):
    ...
    for _ in range(debate_rounds):
        ...
        for agent in [agent1, agent2]:
            ...
            if i == 0:
                prompt = generate_basic_prompt(agent, remaining_items)
                prompt += f"""You initially chose {initial_moves['agent1']} items at first trial by the reason: '{initial_reasonings['agent1']}'.\n
            ...
            if i == 1:
                prompt = generate_basic_prompt(agent, remaining_items)
                prompt += f"""You initially chose {initial_moves['agent2']} items at first trial by the reason: '{initial_reasonings['agent2']}'.\n
